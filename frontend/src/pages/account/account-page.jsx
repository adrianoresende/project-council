...
function getQuotaLabel(plan, amount, t) {
  if (plan === "pro") {
    return amount === 1
      ? t("account.tokenLeftOne")
      : t("account.tokenLeftMany");
  }
...
export default function AccountPage() {
  ...
  const loadSummary = useCallback(async () => {
    ...
    setError("");
    try {
      const [summaryResult, creditsResult] = await Promise.allSettled([
      ...
      if (summaryResult.status === "fulfilled") {
        resolvedSummary = summaryResult.value;
        ...
        try {
          ...
          resolvedSummary = { email: "-", plan: "free" };
          setSummary(resolvedSummary);
          setError(
            summaryResult.reason?.message || t("account.failedToLoadAccount"),
          );
        }
      }
      ...
      if (normalizePlan(resolvedSummary?.plan) === "pro") {
        try {
          const payments = await api.getAccountPayments(1);
          setLatestPayment(
    ...
    const confirmCheckout = async () => {
      ...
      try {
        ...
        const normalizedPlan =
          String(result?.plan || "free").toLowerCase() === "pro"
            ? "pro"
            : "free";
        setSummary((prev) => ({
          ...(prev || {}),
        ...
        window.dispatchEvent(
          new CustomEvent("account-plan-updated", {
            detail: { plan: normalizedPlan },
          }),
        );
        await loadSummary();
      } catch (confirmError) {
  ...
  const handleUpgrade = useCallback(async () => {
    ...
    try {
      ...
      const session = await api.createProCheckoutSession(successUrl, cancelUrl);
      ...
      setError(
        checkoutError.message || t("account.failedStartUpgradeCheckout"),
      );
  ...
  return (
    <div className="h-screen flex-1 overflow-y-auto bg-gradient-to-br from-blue-50 via-sky-50 to-teal-50">
      <div className="mx-auto my-10 w-[calc(100%-32px)] max-w-[720px] rounded-2xl border border-blue-100 bg-white px-6 py-7 shadow-[0_10px_28px_rgba(29,78,156,0.08)]">
        ...
        {error && (
          <div className="mb-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2.5 text-[13px] text-rose-800">
            {error}
          </div>
        )}
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50 px-3.5 py-3">
              <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[15px] font-semibold text-slate-900">
                    {creditsLeft.toLocaleString(language)}{" "}
                    {getQuotaLabel(summary?.plan || "free", creditsLeft, t)}
                  </div>
                </div>
              </div>
            </div>
